...
function getKeysStartingWith(dictionary, letter) {
  ...
  for (const [key, value] of dictionary) {
    const word = Array.from(key);
     if (word[0] === letter) {
      wordArray = [...wordArray, key];
     };
  }
  return wordArray;
...
function checkPresence(dictionary, keys) {
  let presence = [];
  keys.forEach(elem => presence = [...presence, dictionary.has(elem)]);
  return presence;
...
function sortByValues(dictionary) {
  const array = Array.from(dictionary.entries());
  array.sort((elem1, elem2) => {return elem2.at(1) - elem1.at(1)});
  const sortedDictionary = new Map(array); // or new Map(array.values())
